...
FROM chospital INNER JOIN service as s ON chospital.hoscode = s.HOSPCODE
	INNER JOIN drug_opd as d 
	ON chospital.hoscode = d.HOSPCODE AND d.HOSPCODE = s.HOSPCODE
	AND d.PID = s.PID AND d.SEQ = s.SEQ AND d.DATE_SERV = s.DATE_SERV
	INNER JOIN diagnosis_opd as da ON chospital.hoscode = da.HOSPCODE
	AND da.HOSPCODE = s.HOSPCODE AND da.HOSPCODE = d.HOSPCODE
	AND da.PID = s.PID AND da.PID = d.PID
	AND da.SEQ = s.SEQ AND da.SEQ = d.SEQ
	AND da.DATE_SERV = s.DATE_SERV AND da.DATE_SERV = d.DATE_SERV

WHERE chospital.hostype IN ('13','18')
